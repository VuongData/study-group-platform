...
const Login = ({ onSwitchToRegister, onSwitchToForgot }) => {
  ...
  // State biểu cảm: 'normal', 'happy', 'scared', 'surprised'
  ...
  // 👇 Logic Spotlight: Cập nhật biến CSS --x, --y theo chuột
  ...
  return (
    <div 
      ref={containerRef}
      className={`login-lamp-container ${!isLampOn ? 'dark-room' : ''}`}
      onMouseMove={handleMouseMove} // Bắt sự kiện chuột
    >
      {/* --- LAMP ANIMATION --- */}
      <div className={`lamp-wrapper ${!isLampOn ? 'lamp-off' : ''}`}>
        <div className="wire"></div>
        <div className="lamp-head">
          
          {/* Biểu cảm khuôn mặt thay đổi theo state */}
          <div className={`lamp-face ${expression}`}>
            <div className="eye left"></div><div className="eye right"></div><div className="mouth"></div>
          </div>
          
          {/* Tia sáng mờ đi khi đang sợ (nhập pass) */}
          <div className="light-beam" style={{ opacity: expression === 'scared' ? 0.2 : 1 }}></div>
      ...
      <div className="login-box">
        ...
        <form onSubmit={handleLogin}>
          ...
          <div className="input-group">
            ...
            <input 
              ...
              onFocus={() => setExpression('scared')} 
              onBlur={() => setExpression('normal')}
            />
            {/* Show pass -> Ngạc nhiên */}
            <button 
              type="button" className="btn-toggle-password" tabIndex="-1"
              onClick={() => {
                setShowPassword(!showPassword);
                setExpression(!showPassword ? 'surprised' : 'scared');
              }}
              onMouseEnter={() => setExpression('surprised')}
              onMouseLeave={() => setExpression(password ? 'scared' : 'normal')}
            >
          ...
          <div className="forgot-pass">
            <button 
              type="button" onClick={onSwitchToForgot}
              style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: '0.85rem', padding: 0, fontFamily: 'inherit', transition: '0.3s' }}
        ...
        <div className="footer">
          Chưa có tài khoản? <button onClick={onSwitchToRegister}>Đăng ký ngay</button>
        </div>
      </div>
    </div>
  );
